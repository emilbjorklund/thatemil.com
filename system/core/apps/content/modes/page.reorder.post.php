        ...
        function render_tree($Pages, $CurrentUser, $parentID=0, $class=false)
        {
            ...
            if (PerchUtil::count($pages)) {
                ...
                foreach($pages as $Page) {
                    $s .= '<li id="page_'.$Page->id().'" '.(!$Page->role_may_create_subpages($CurrentUser)?' class="ui-nestedSortable-no-nesting "':'class=""').'><div class="page icon">';
                    $s .= '<input type="text" name="p-'.$Page->id().'" value="'.$Page->pageOrder().'" />';
                    $s .= ''.PerchUtil::html($Page->pageTitle()).'</div>';
                    
                    $s .= render_tree($Pages, $CurrentUser, $Page->id(), $class);
                    $s .= '</li>';
                }
