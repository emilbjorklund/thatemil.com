...
class PerchContent_Pages extends PerchFactory
{
    ...
    public function get_navigation($opts, $current_page)
    {
        ...
        // cache page list
        if ($navgroup) {
            $sql = 'SELECT np.pageID, np.pageParentID, p.pagePath, p.pageTitle, p.pageNavText, p.pageNew, p.pageOrder, np.pageDepth, p.pageSortPath, np.pageTreePosition, p.pageAccessTags
